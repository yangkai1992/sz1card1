...
namespace WebUserControl.UI
{
    ...
    public class WapDataPager : WebControl, IPostBackEventHandler
    {
        ...
        private string nexPageText = "下一页";
       ...
       public string NextPageText
       {
        ...
        {
            string url = DesignMode ? "a.aspx" : Page.Request.RawUrl.ToString();
            if (url.IndexOf("?") > 0)
            {
            ...
            if (PageIndex == 0)
            {
                switch (PageStyle)
                {
                    ...
                    case PageStyle.Standard:
                        ...
                        writer.Write("<span class='disabled'>"+previousPageText+"</span>");
                        break;
                }
            }
            else
            {
                ...
                {
                    case PageStyle.Classic:
                        writer.Write("<a href=\"" + url+"="+PageIndex + "\"> "+previousPageText+" &nbsp;&nbsp;</a>");
                        break;
                    case PageStyle.Standard:
                        writer.Write("<a href=\"" + string.Format(GetLinkScript(), url, 1) + "\"> 首页 </a>");
                        writer.Write("<a href=\"" + string.Format(GetLinkScript(), url, PageIndex) + "\">" + previousPageText + "</a>");
                        break;
                }
            }
            ...
            if (PageIndex == lastIndex)
            {
                switch (PageStyle)
                {
                    case PageStyle.Classic:
                        ...
                        break;
                    case PageStyle.Standard:
                        writer.Write("<span class='disabled'>"+nexPageText+"</span>");
            ...
            else
            {
                ...
                {
                    case PageStyle.Classic:
                        writer.Write("<a href=\"" + string.Format(GetLinkScript(), url, PageIndex + 2) + "\"> " + nexPageText + " </a>");
                        break;
                    case PageStyle.Standard:
                        writer.Write("<a href=\"" + string.Format(GetLinkScript(), url, PageIndex + 2) + "\">"+nexPageText+"</a>");
