...
namespace WebUserControl.UI
{
    ...
    ToolboxData("<{0}:TabOption runat=\"server\"></{0}:TabOption>"),
    ParseChildren(typeof(TabOptionItem)),
    AspNetHostingPermission(SecurityAction.LinkDemand, Level = AspNetHostingPermissionLevel.Minimal),
    AspNetHostingPermission(SecurityAction.InheritanceDemand,
        Level = AspNetHostingPermissionLevel.Minimal)]
    public class TabOption : Control
    {
        ...
        @"<TD {4}>
	        <TABLE height=22 cellSpacing=0 cellPadding=0 border=0 style='cursor:default'>
		        <TBODY>
                       <TR>
				         <TD width=3><IMG id=tabImgLeft__{1} height=22 src='{0}tab_unactive_left.gif'  width=3></TD>
				         <TD class=tab id=tabLabel__{1} onclick=""javascript:tabClick({1},{2},'{5}');{6}"" background='{0}tab_unactive_bg.gif' UNSELECTABLE='on'>{3}</TD>
        ...
        private void CreateTabHtml()
        {
            ...
            if (TaboptionItems.Count > 0)
            {
                ...
                for (int i = 0; i < TaboptionItems.Count; i++)
                {
                    ...
                    sbTabButton.AppendFormat(_TabButton, TabImagesPath, i, TaboptionItems.Count, TaboptionItems[i].Tab_Name, IsDisp, TaboptionItems[i].Focus_Name, TaboptionItems[i].OnClientClick);
                }
            }
        ...
        function tabClick(idx,count,focusname) {1}
          for (i_tr = 0; i_tr < count; i_tr++) {1}
            if (i_tr == idx) {1}
              var tabImgLeft = eval(document.getElementById('tabImgLeft__'+idx));
              var tabImgRight = eval(document.getElementById('tabImgRight__'+idx));
              var tabLabel = eval(document.getElementById('tabLabel__'+idx));
              var tabContent = eval(document.getElementById('tabContent__'+idx));
              ...
              continue;
            {2}
            var tabImgLeft = eval(document.getElementById('tabImgLeft__'+i_tr));
            var tabImgRight = eval(document.getElementById('tabImgRight__'+i_tr));
            var tabLabel = eval(document.getElementById('tabLabel__'+i_tr));
            var tabContent = eval(document.getElementById('tabContent__'+i_tr));
        ...
        private string _HiddenInputName = "pqmagic_SelectIndex";
        private string _HiddenSelectIndex = "<input type='hidden' name='{0}' value='{1}'>";
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            cs.RegisterStartupScript(typeof(string), "TabJs", string.Format(_TabJs, TabImagesPath, "{", "}", SelectIndex, TaboptionItems.Count, _HiddenInputName, TaboptionItems[SelectIndex].Focus_Name));
        }
